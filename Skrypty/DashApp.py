...
app.layout = (
html.Div(style={"backgroundColor": "#1e1e1e"}, children = [html.Link(rel='stylesheet', href='/assets/style.css'),
    dcc.Tabs([
        dcc.Tab(label='Założenia', style={'backgroundColor': '#1e1e1e', 'color': 'white'},selected_style = {'backgroundColor': '#1e1e1e', 'color': 'white', 'borderTop': '4px solid #fec036'}, children=[
            dbc.Container([
                dbc.Row([
                        ...
                        dbc.Col([
                            ...
                            #html.Button('Oblicz', id='oblicz-button', n_clicks=0),
                        ...
                        dbc.Col([
                            ...
                            dcc.Slider(id='year_slider', value=cfg.start_year, min=cfg.start_year, max=cfg.end_year, step=1, marks ={int(y):str(int(y)) for y in np.arange(cfg.start_year, cfg.end_year+1)}),
                            dcc.Dropdown(options=[
                                {'label': 'GWh', 'value': 1e3},
                            ...
                            dcc.Graph(id="podaz-popyt-figure"),
                            html.Div(id='output-container', style={'margin-top': '20px'}),

                            dcc.Graph(id="residual-figure"),
                ...
                children=[
                    dbc.Container([
                        ...
                        dbc.Row([
                            ...
                            dcc.Graph(id="year-barplots-figure"),
                            html.H3("Bilans mocy", style={"textAlign": "center", 'color': "#fec036"}),
                            dcc.Graph(id="year-moce-figure"),
                    ...
                    dcc.Tab(label='Godzinowa symulacja magazynów', style={'backgroundColor': '#1e1e1e', 'color': 'white'},selected_style = {'backgroundColor': '#1e1e1e', 'color': 'white', 'borderTop': '4px solid #fec036'}, children=[
            dbc.Container([

                                dbc.Row(dcc.Slider(id='year-slider-secondary', value=cfg.start_year, min=cfg.start_year, max=cfg.end_year, step=1, marks ={int(y):str(int(y)) for y in np.arange(cfg.start_year, cfg.end_year+1)}),
),

                dbc.Row([
                                html.H3("Suma pracy i stanu naładowania magazynów", style={"textAlign": "center", 'color': "#fec036"}),
                                dcc.Graph(id="opt-storage-figure"),
                                html.H3("Magazyny złożowe", style={"textAlign": "center", 'color': "#fec036"}),
                                dcc.Graph(id="sanok-storage-figure"),
                                dcc.Graph(id="wierzchowice-storage-figure"),
                                html.H3("Magazyny kawernowe", style={"textAlign": "center", 'color': "#fec036"}),
                                dcc.Graph(id="kosakowo-storage-figure"),
                                dcc.Graph(id="mogilno-storage-figure"),
                                dcc.Graph(id="damasławek-storage-figure")
