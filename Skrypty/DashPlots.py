...
@app.callback(
    ...
    #Output("download-ts-results-csv", "data"),
...
def plot_all_years_bars(popyt_ccgt_scen_name, import_selector, n_clicks, podaz_df, zrodla, storage_investment_scenario):
    ...
    if ctx.triggered_id == 'run-all_year-calc':
        ...
        results_df, all_gas_demands, storage_results_df, all_years_h = run_all_years_sim(popyt_ccgt_scen_name, import_selector, podaz_df, zrodla, storage_investment_scenario)

        ts_save = pd.merge(storage_results_df, all_years_h, left_on=cfg.datetime_col_name, right_on=cfg.datetime_col_name)
        #ts_save.to_csv('../Wyniki/results_ts_%s_wint_imp_%s_infr_%s.csv'%(scena_name, import_selector, cfg.infr_name))

        #results_df.T.to_excel('../Wyniki/results_df_%s_wint_imp_%s_infr_%s.xlsx'%(scena_name, import_selector, cfg.infr_name))
        results_df_excel = results_df.T
        #
        results_long = pd.melt(results_df.reset_index(names='rok'), id_vars='rok', value_vars=[ 'dostarczony gaz z magazynów', 'niedostarczony gaz z magazynów'] )
        results_long['rok'] = results_long['rok'].astype(str)

        fig4 = px.bar(results_long, x="rok", y="value",color='variable', text_auto='.2s', title="zapotrzebowanie i produkcja z magazynów")#, color_discrete_map = {'dostarczony gaz z magazynów':'#1f77b4', 'niedostarczony gaz z magazynów':'#d62728'})
        fig4.update_layout(main_layout)
        fig4.update_xaxes(showgrid=False, showline=False)
        fig4.update_yaxes(showgrid=False)
        fig4.update_layout(yaxis_title='MWh')

        moce_long = pd.melt(results_df.reset_index(names='rok'), id_vars='rok', value_vars=['maksymalna moc magazynów', 'zapotrzebowanie na moc w magazynach'])
        moce_long['rok'] = moce_long['rok'].astype(str)
        fig5 = px.line(moce_long, x="rok", y="value", color='variable',
                      title="Wystarczalność mocy")  # , color_discrete_map = {'dostarczony gaz z magazynów':'#1f77b4', 'niedostarczony gaz z magazynów':'#d62728'})
        ...
        return fig4, fig5, storage_results_df.to_dict('records'), dcc.send_data_frame(results_df_excel.to_excel, 'yearly_%s_wint_imp_%s_infr_%s.xlsx' % (popyt_ccgt_scen_name, import_selector, storage_investment_scenario))#, dcc.send_data_frame(ts_save.to_csv, 'hourly_%s_wint_imp_%s_infr_%s.csv' % (popyt_ccgt_scen_name, import_selector, storage_investment_scenario))
    else:
        raise PreventUpdate
...
def plot_residual(residual, units, u_label):
    ...
    residual.loc[:, cfg.datetime_col_name] = pd.to_datetime(residual[cfg.datetime_col_name], format=cfg.datetime_format_dash)
    residual.loc[:, cfg.demand_name] = (residual.loc[:, cfg.demand_name] / units).round(2)
    fig = px.line(residual, x=cfg.datetime_col_name, y=cfg.demand_name, title=cfg.demand_name, color_discrete_sequence=['darkorange'])
...
def plot_podaz_popyt(year_df, residual, scen_name, units, opt):
    ...
    #chosen_demand = [ cfg.przemysl_name, cfg.dystrybucja_name, cfg.ec_name,  scen_name]
    ...
    fig = px.area(plot_df, x=cfg.datetime_col_name, y='value',color="variable", category_orders = {'variable' : chosen_demand})
...
@app.callback(
    ...
    Input(component_id='year-slider-secondary', component_property='value'),
    Input(component_id='units-selector', component_property='value'),
    State(component_id='units-selector', component_property='options'),
    Input(component_id='storage-investment-scenario', component_property='value'),
)
def plot_opt_storage(storage_results_table, year, units, opt, storage_investment_scenario):
    ...
    if ctx.triggered_id != None and ctx.triggered_id != 'datatable-magazyny' and len(pd.DataFrame(storage_results_table))>0:
        u_label = [x['label'] for x in opt if x['value'] == units][0]
        ...
        results_df.loc[:, cfg.datetime_col_name] = pd.to_datetime(results_df.loc[:, cfg.datetime_col_name], format= cfg.datetime_format_dash)
        results_df = results_df.set_index(cfg.datetime_col_name)
        y_mask =results_df['rok'] == year
        results_df = results_df[y_mask]

        # Magazyny po kolei zdejmują zapotrzebowania z całkowitego zapotrzebowania do magazynów. Zapotrzebowanie do pierwszego w kolejności magazynu jest całkowitym zapotrzebowaniem
...
def make_storage_fig(results_df, title, magazyny_y_df, units, u_label, year):
    ...
    fig.add_trace(go.Scatter(x=results_df.index,y=residual,
        mode='lines', name = 'Zap. do magazynów'), row=1, col=1)

    fig.add_trace(go.Scatter(x=results_df.index, y=pp,
        mode='lines', name = 'Punkty pracy'), row=1, col=1)

    fig.add_trace(go.Scatter(x=results_df.index,y=SoC,
        mode='lines', name='Stan naładowania'), row=2, col=1)

    fig.add_hline(y=magazyny_y_df.loc[title, cfg.moc_odb_col]/units, name = 'moc odbioru i zatłaczania', line_width=1, line_dash="dash", line_color="white", row=1, col=1, showlegend=True)
    fig.add_hline(y=magazyny_y_df.loc[title, cfg.moc_zatl_col]*-1/units, line_width=1, line_dash="dash", line_color="white", row=1, col=1)
    fig.add_hline(y=magazyny_y_df.loc[title, cfg.pojemnosc_col]/units,  name = 'pojemność',line_width=1, line_dash="dash", line_color="white", row=2, col=1, showlegend=True)
    if title in cfg.mag_calc_order:
        fig.add_hline(y=magazyny_y_df.loc[title, cfg.pojemnosc_col]/units * rezerwy_df.loc[year, title],  name = 'pojemność',line_width=1, line_dash="dash", line_color="white", row=2, col=1, showlegend=True)
...
def annotate_plot(relayoutData, year_dt, units, opt):
    ...
    filtered_df = year_df[(year_df[cfg.datetime_col_name] >= start_x) &
                           (year_df[cfg.datetime_col_name] <= end_x)]
...
def annotate_residual(relayoutData, residual_dt, units, opt):
    ...
    start_x, end_x = relayoutData['xaxis.range[0]'], relayoutData['xaxis.range[1]']
        # Extract the selected points
    # Filter the DataFrame based on the x-axis range
    filtered_df = residual_series[(residual_series[cfg.datetime_col_name] >= start_x) &
                           (residual_series[cfg.datetime_col_name] <= end_x)]
